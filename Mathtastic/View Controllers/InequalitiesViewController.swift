...
class InequalitiesViewController: UIViewController{
    ...
    var pointstext = ""
    var result = ""
    let questions:[String] = ["5x=25\nSolve for x","7x+5=54;Solve for x","6x=34;Solve for x"]
    ...
    @IBAction func button3(_ sender: Any) {
        if((sender as AnyObject).tag == Int(rightAnswerPlacement)){
            ...
            result="Correct"
            points += 1
            self.performSegue(withIdentifier: "popup3", sender: nil)
        ...
        func newQuestion()
        {
         questionLabel3.text = questions[currentQuestion]
